 ...
 *
  ...
  def parseGlobalExpressionsAsync(cmwellRDFHelper: CMWellRDFHelper, crudServiceFS: CRUDServiceFS)(models: Seq[(Option[String],Model)], urlToLast: Map[String,String], metaNsInfotonMap: InfotonRepr): Future[ParsingResponse] = {
...
//        }.toList
  ...
  def updateDeleteValuesMap(cmwellRDFHelper: CMWellRDFHelper, deleteValuesMap: MMap[String,Map[String,Set[FieldValue]]], infotonPath: String, pvl: List[(Property,RDFNode)], quad: Option[String]): Unit = {
    pvl.foreach{
      case (p,v) => {
        val cmwAttribute = getCmwellFieldNameForUrl(cmwellRDFHelper, p.getNameSpace, Some(p.getLocalName)).get //TODO: protect against empty Option
